...
import { getAccountInfo, getCourses, getMyCourses, getPanToken, userLogin } from './functions/user';
...
// [定制功能] 我的课程：返回当前用户全部课程（按凭据对应的用户过滤）
router.post('/courses', async (ctx) => {
  const { uid, _d, vc3 } = ctx.request.body as any;
  const courses = await getMyCourses(uid, _d, vc3);
  // 返回：课程数组 | 'AuthFailed' | 'ParseError'
  ctx.body = courses;
});
