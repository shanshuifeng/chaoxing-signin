...
/**
 * [定制功能] 拍照签到：确保超星云盘根目录存在 0.jpg/0.png
 * - 已存在：直接返回 objectId，正常签到
 * - 缺失：主动提醒用户上传（控制台日志 + 尝试超星IM消息通知），
 *   并轮询等待上传（每15秒检查一次，最长10分钟），上传后自动完成签到
 */
async function ensurePhotoObjectId(params: UserCookieType & { tuid: string; }): Promise<string | null> {
  let objectId = await getObjectIdFromcxPan(params);
  if (objectId !== null) return objectId;

  const REMIND_MSG = [
    '【拍照签到提醒】检测到拍照签到，但您的超星云盘根目录还没有照片！',
    '上传路径：浏览器打开 https://pan-yz.chaoxing.com （超星云盘）',
    '操作方式：登录后进入【云盘】，将照片上传到【根目录】，并重命名为 0.jpg 或 0.png',
    '上传成功后系统将自动完成本次拍照签到（持续等待10分钟），无需其他操作。',
  ].join('\n');
  console.log(red(REMIND_MSG));

  // 尽力通过超星 IM 给用户本人发一条提醒消息（手机学习通可收到），失败不影响主流程
  try {
    if (params.tuid) {
      const msg = new webIM.message('txt', Date.now());
      msg.set({ msg: REMIND_MSG, to: params.tuid, chatType: 'singleChat' });
      conn.send(msg.body);
      console.log(blue('已通过超星IM向本人发送上传提醒'));
    }
  } catch (e) {
    console.warn('[拍照提醒] IM消息发送失败（不影响等待上传流程）:', e);
  }

  // 轮询等待上传：40次 x 15秒 = 10分钟
  for (let i = 0; i < 40; i++) {
    await delay(15);
    objectId = await getObjectIdFromcxPan(params);
    if (objectId !== null) {
      console.log(blue('【拍照签到】检测到照片已上传，继续完成签到'));
      return objectId;
    }
  }
  console.log(red('【拍照签到】等待超时（10分钟内未检测到上传），本次拍照签到放弃'));
  return null;
}
...
async function Sign(realname: string, params: UserCookieType & { tuid: string; }, config: any, activity: Activity) {
  ...
  if (!activity.courseId) {
    ...
    switch (activityType) {
      ...
      case 'photo': {
        const objectId = await ensurePhotoObjectId(params);
...
(async () => {
  ...
  console.log(blue('拍照签到说明：自动签依赖超星云盘根目录的 0.jpg/0.png；若缺失，触发时将主动提醒上传并等待（10分钟内有效）'));
})();
