...
function DashBoard() {
  ...
  const start = async () => {
    ...
    switch (activity) {
      ...
      case 'NetworkError': setSign({ activity: { name: '网络异常，请重试' }, status: '' }); break;
  ...
  return (
    <div>
      {/* [定制功能] 我的课程入口 */}
      <ButtonBase
        onClick={() => { navigate('/mycourses/' + params.phone); }}
        sx={{ borderRadius: 50, position: 'absolute', top: 28, right: 32, height: '56px', width: '56px' }}
        className='neum-button'
        disableRipple
      >
        <span style={{ fontSize: '0.9rem' }}>课程</span>
      </ButtonBase>
      {
        control.start.show &&
        <ButtonBase
